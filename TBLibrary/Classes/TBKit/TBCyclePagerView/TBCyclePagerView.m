...
NS_INLINE BOOL TBEqualIndexSection(TBIndexSection indexSection1,TBIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE TBIndexSection TBMakeIndexSection(NSInteger index, NSInteger section) {
    TBIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface TBCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, TBCyclePagerTransformLayoutDelegate> {
    struct {
...
@property (nonatomic, assign) TBIndexSection indexSection; // current index
@property (nonatomic, assign) NSInteger dequeueSection;
@property (nonatomic, assign) TBIndexSection beginDragIndexSection;
...
@implementation TBCyclePagerView
...
- (void)addCollectionView {
    TBCyclePagerTransformLayout *layout = [[TBCyclePagerTransformLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:TBPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (TBCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<TBCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((TBCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<TBCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(TBPagerScrollDirection)direction animate:(BOOL)animate {
    TBIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
    [self scrollToItemAtIndexSection:indexSection animate:animate];
}

- (void)scrollToItemAtIndex:(NSInteger)index animate:(BOOL)animate {
    if (!_isInfiniteLoop) {
        [self scrollToItemAtIndexSection:TBMakeIndexSection(index, 0) animate:animate];
        return;
    }

    [self scrollToItemAtIndexSection:TBMakeIndexSection(index, index >= self.curIndex ? _indexSection.section : _indexSection.section+1) animate:YES];
}

- (void)scrollToItemAtIndexSection:(TBIndexSection)indexSection animate:(BOOL)animate {
...
- (void)updateLayout {
    ...
    ((TBCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(TBIndexSection)indexSection {
...
- (TBIndexSection)nearlyIndexPathAtDirection:(TBPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (TBIndexSection)nearlyIndexPathForIndexSection:(TBIndexSection)indexSection direction:(TBPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == TBPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? TBMakeIndexSection(0, 0) : indexSection;
        } else if (direction == TBPagerScrollDirectionRight) {
            return TBMakeIndexSection(indexSection.index+1, 0);
        }
        
        if (indexSection.index == 0) {
            return _autoScrollInterval > 0 ? TBMakeIndexSection(_numberOfItems - 1, 0) : indexSection;
        }
        return TBMakeIndexSection(indexSection.index-1, 0);
    }
    
    if (direction == TBPagerScrollDirectionRight) {
        if (indexSection.index < _numberOfItems-1) {
            return TBMakeIndexSection(indexSection.index+1, indexSection.section);
        }
        if (indexSection.section >= kPagerViewMaxSectionCount-1) {
            return TBMakeIndexSection(indexSection.index, kPagerViewMaxSectionCount-1);
        }
        return TBMakeIndexSection(0, indexSection.section+1);
    }
    ...
    if (indexSection.section <= 0) {
        return TBMakeIndexSection(indexSection.index, 0);
    }
    return TBMakeIndexSection(_numberOfItems-1, indexSection.section-1);
}

- (TBIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
        return TBMakeIndexSection(0, 0);
    }
    ...
    return TBMakeIndexSection(curIndex, curSection);
}

- (CGFloat)caculateOffsetXAtIndexSection:(TBIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)resetPagerViewAtIndex:(NSInteger)index {
    ...
    [self scrollToItemAtIndexSection:TBMakeIndexSection(index, _isInfiniteLoop ? kPagerViewMaxSectionCount/3 : 0) animate:NO];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    TBIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    TBIndexSection indexSection = _indexSection;
    ...
    if (_delegateFlags.didScrollFromIndexToNewIndex && !TBEqualIndexSection(_indexSection, indexSection)) {
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    if (fabs(velocity.x) < 0.35 || !TBEqualIndexSection(_beginDragIndexSection, _indexSection)) {
        targetContentOffset->x = [self caculateOffsetXAtIndexSection:_indexSection];
        return;
    }
    TBPagerScrollDirection direction = TBPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = TBPagerScrollDirectionLeft;
    }
    TBIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - TBCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((TBCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
