...
typedef NS_ENUM(NSUInteger, TBTransformLayoutItemDirection) {
    TBTransformLayoutItemLeft,
    TBTransformLayoutItemCenter,
    TBTransformLayoutItemRight,
};


@interface TBCyclePagerTransformLayout () {
    struct {
...
@interface TBCyclePagerViewLayout ()
...
@implementation TBCyclePagerTransformLayout

#pragma mark - getter setter

- (void)setDelegate:(id<TBCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(TBCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (TBTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    TBTransformLayoutItemDirection direction= TBTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = TBTransformLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = TBTransformLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == TBCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    NSArray *attributesArray = [super layoutAttributesForElementsInRect:rect];
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != TBCyclePagerTransformLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(_layout.layoutType != TBCyclePagerTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(TBCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case TBCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case TBCyclePagerTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(TBCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case TBCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        TBTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case TBTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case TBTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation TBCyclePagerViewLayout
