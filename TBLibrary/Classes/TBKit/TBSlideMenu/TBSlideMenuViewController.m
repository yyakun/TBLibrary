...
@interface TBSlideMenuViewController () <UIGestureRecognizerDelegate> {
    //记录起始位置
...
@implementation TBSlideMenuViewController
...
- (void)pan:(UIPanGestureRecognizer*)pan {
    if ([self.delegate respondsToSelector:@selector(enablePanGusture:)]) {
        if (![self.delegate enablePanGusture:self]) {
            return;
        }
    }
    switch (pan.state) {
...
- (void)tap {
    if ([self.delegate respondsToSelector:@selector(enableTapGusture:)]) {
        if (![self.delegate enableTapGusture:self]) {
            return;
        }
    }
    [self showRootViewControllerAnimated:true];
...
- (void)showRootViewControllerAnimated:(BOOL)animated {
    if ([self.delegate respondsToSelector:@selector(shouldShowRootViewController:)]) {
        if (![self.delegate shouldShowRootViewController:self]) {
            return;
        }
    }
    
    if ([self.delegate respondsToSelector:@selector(willShowRootViewController:)]) {
        [self.delegate willShowRootViewController:self];
    }
...
- (void)showLeftViewControllerAnimated:(BOOL)animated {
    ...
    if ([self.delegate respondsToSelector:@selector(shouldShowLeftViewController:)]) {
        if (![self.delegate shouldShowLeftViewController:self]) {
            return;
        }
    }
    if ([self.delegate respondsToSelector:@selector(willShowLeftViewController:)]) {
        [self.delegate willShowLeftViewController:self];
    }
...
- (void)showRightViewControllerAnimated:(BOOL)animated {
    ...
    if ([self.delegate respondsToSelector:@selector(shouldShowRightViewController:)]) {
        if (![self.delegate shouldShowRightViewController:self]) {
            return;
        }
    }
    if ([self.delegate respondsToSelector:@selector(willShowRightViewController:)]) {
        [self.delegate willShowRightViewController:self];
    }
